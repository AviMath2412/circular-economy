...
export default function ProductCard({ product }) {
  ...
  return (
    <Link
      ...
      className="group block bg-white border border-forest/15 rounded-xl p-5 hover:border-forest/50 hover:shadow-md transition-all duration-200"
    >
      <div className="flex justify-between items-start mb-2">
        <h3 className="font-semibold text-forest-dark group-hover:text-forest transition line-clamp-1">
          {product.name}
        </h3>
        <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${getScoreBadgeColor(product.condition_score)}`}>
